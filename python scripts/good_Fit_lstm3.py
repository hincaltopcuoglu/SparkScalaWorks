# -*- coding: utf-8 -*-
"""
Created on Mon Nov 20 09:25:22 2017
...
# Part 3 - Making the predictions and visualising the results# Getting the real stock price for February 1st 2012 - January 31st 2017
...
real_stock_price = np.concatenate((training_set[0:1426], test_set), axis = 0)

# Getting the predicted stock price of 2017
scaled_real_stock_price = sc.fit_transform(real_stock_price)
inputs = []
for i in range(1426, 1453):
    inputs.append(scaled_real_stock_price[i-28:i, 0])
...
plt.plot(real_stock_price[1426:], color = 'red', label = 'Real KEAMA Stock Price')
plt.plot(predicted_stock_price, color = 'blue', label = 'Predicted KEAMA Stock Price')
plt.title('KEAMA Stock Price Prediction')
plt.xlabel('Time')
plt.ylabel('KEAMA Stock Price')
plt.legend()
plt.show()
plt.show()
...
predictions = predict_sequences_multiple(regressor,inputs,1,30)


inversed2=scaler.inverse_transform(np.array(predictions[29]).reshape(-1, 1))

dataset_test = pd.read_csv('C:\\Users\\Administrator\\Desktop\\feolad_test3.txt')
...
x2.to_csv("C:\\Users\\Administrator\\Desktop\\KEAMA_Share_Predictions_20112017.csv",sep="\t",float_format='%10.2f')
