...
object MovieSimilarities1M {
  ...
  def loadMovieNames() : Map[Int, String] = {
    ...
    var movieNames:Map[Int, String] = Map()
     ...
     val lines = Source.fromFile("C:/Users/htopcuoglu/Desktop/SparkScala/movies.dat").getLines()
     for (line <- lines) {
  ...
  def main(args: Array[String]) {
    ...
    conf.setAppName("MovieSimilarities1M").setMaster("local[*]")
    val sc = new SparkContext(conf)
    
    println("\nLoading movie names...")
    val nameDict = loadMovieNames()
    
    val data = sc.textFile("s3a://sundog-spark/ml-1m/ratings.dat")
